...
#ifndef TIDBDIALECT_H
#define TIDBDIALECT_H

#include "Dialect.h"

class TiDBDialect : public Dialect {
	...
	private:
		...
		std::vector<const char*> createSchemaStatements = {
			...
			"	PRIMARY KEY (W_ID)\n"
			")",
			"alter table tpcch.warehouse set tiflash replica 1",
			...
			"alter table tpcch.district set tiflash replica 1",
			...
			"alter table tpcch.customer set tiflash replica 1",
			...
			"alter table tpcch.history set tiflash replica 1",
			...
			")",
			"alter table tpcch.neworder set tiflash replica 1",
			...
			") ",
			"alter table tpcch.order set tiflash replica 1",
			...
			") ",
			"alter table tpcch.orderline set tiflash replica 1",
			...
			"	PRIMARY KEY(I_ID)\n"
			") ",
			"alter table tpcch.item set tiflash replica 1",
			...
			") ",
			"alter table tpcch.stock set tiflash replica 1",
			...
			") ",
			"alter table tpcch.nation set tiflash replica 1",
			...
			") ",
			"alter table tpcch.supplier set tiflash replica 1",
			...
			") ",
			"alter table tpcch.region set tiflash replica 1",
		};

		std::vector<const char*> additionalPreparationStatements = {
			"analyze table TPCCH.WAREHOUSE",
			"analyze table TPCCH.DISTRICT",
			"analyze table TPCCH.CUSTOMER",
			"analyze table TPCCH.HISTORY",
			"analyze table TPCCH.NEWORDER",
			"analyze table TPCCH.ORDER",
			"analyze table TPCCH.ORDERLINE",
			"analyze table TPCCH.ITEM",
			"analyze table TPCCH.STOCK",
			"analyze table TPCCH.NATION",
			"analyze table TPCCH.SUPPLIER",
			"analyze table TPCCH.REGION",
		};
